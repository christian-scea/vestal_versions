module VestalVersions
  ...
  module Reversion
    ...
    module InstanceMethods
      # Returns the current version numb for the versioned object.
      def version
      ...
      #
      # The version numb of the object will reflect whatever version has been reverted to, and
      # the return value of the +revert_to+ method is also the target version numb.
      def revert_to(value)
        to_numb = versions.numb_at(value)

        changes_between(version, to_numb).each do |attribute, change|
          write_attribute(attribute, change.last)
        end

        reset_version(to_numb)
      end
      ...
      private
        ...
        # Returns the numb of the last created version in the object's version history.
        #
        # If no associated versions exist, the object is considered at version 1.
        def last_version
          @last_version ||= versions.maximum(:numb) || 1
        end

        # Clears the cached version numb instance variables so that they can be recalculated.
