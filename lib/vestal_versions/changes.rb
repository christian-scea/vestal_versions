module VestalVersions
  ...
  module Changes
    ...
    module InstanceMethods
      ...
      # version numb, a symbol representing an association proxy method, a string representing a
      # version tag or a version object itself.
      def changes_between(from, to)
        from_numb, to_numb = versions.numb_at(from), versions.numb_at(to)
        return {} if from_numb == to_numb
        chain = versions.between(from_numb, to_numb).reject(&:initial?)
        return {} if chain.empty?

        backward = from_numb > to_numb
        backward ? chain.pop : chain.shift unless from_numb == 1 || to_numb == 1
